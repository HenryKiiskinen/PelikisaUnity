...
namespace UnityStandardAssets._2D
{
    public class PlatformerCharacter2D : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (this.GetComponent<Transform>().position.y <= -45.0f)
            {
                Vector2 apu = this.GetComponent<Transform>().position;
                apu.y = 40f;
                this.GetComponent<Transform>().position = apu;
            }
            if (this.GetComponent<Transform>().position.y >= 60.0f)
            {
                Vector2 apu = this.GetComponent<Transform>().position;
                apu.y = -15f;
                this.GetComponent<Transform>().position = apu;
